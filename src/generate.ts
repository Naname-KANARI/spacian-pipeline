...
function buildPrompt(candidate: Candidate, snippet: string | undefined, vocabTiers: VocabTiers): string {
  ...
  - 文体は全員常体（だ・である調）。ただし上記キャラクター指示に従い、常体の範囲内で語尾・言葉遣いの個性を出すこと（「だよ」「なんだ」「〜ね」等可。「です」「ます」は不可）
...
subtitle: 3行を改行（\\n）で区切る。各行24字以内・体言止め推奨。3行全体が起承結の論理展開になること。
  1行目: 事実 — 何が起きたか（起）
  2行目: 重要性 — なぜ注目されるか（承）
  3行目: 視点 — 今後どこを見るべきか（結）（結論ではなく観測の方向性）

blocks:
  - label: "facts"
    content: ニュースの詳細（事実のみ、小見出しなし・連続した文章、300字以内・読了1分以内）
             何が起きたか・いつ・誰が・背景を簡潔に。意見を含めない。
  - label: "analysis"
...
- blocks（facts/analysis/note）はすべて常体（だ・である調）で統一すること。foursDialogueはペルソナのキャラクター指示に従った個性を維持しつつ常体で書く。「です」「ます」は記事全体で使わない
